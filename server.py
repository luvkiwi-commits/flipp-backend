...
SEARCH_TERMS = {
    "publix": ["publix chicken", "publix produce", "publix beef", "publix seafood",
               "publix organic", "publix dairy", "publix bogo", "publix sale"],
    "kroger": ["kroger chicken", "kroger produce", "kroger beef", "kroger organic",
               "kroger dairy", "kroger sale", "kroger seafood", "kroger deli"],
}
        ...
        for term in terms:
            try:
                ...
                items = data if isinstance(data, list) else next(
                    (data[k] for k in ("items","results","data","flyer_items")
...
,'')):.2f}" if orig else None,
                        ...
                        "deal_type": dtype,
                        "savings_pct": pct,
                        "merchant": merchant or store,
                    })
            except Exception:
                continue

        # If store filtering removed everything, retry without merchant filter
        if not deals:
            seen2 = set()
            for term in terms[:3]:
                try:
...
,'')):.2f}" if orig else None,
                            ...
                            "merchant": (item.get("merchant_name") or store).lower(),
                        })
                except Exception:
                    continue
        ...
        return jsonify({"store": store, "count": len(deals), "items": deals})
